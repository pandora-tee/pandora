...
class SimMemcpy(SimProcedure):
    ...
    def run(self, **kwargs):
        ...
        if buffer_entirely_inside_enclave(self.state, dst, size) \
                and buffer_entirely_inside_enclave(self.state, src, size):
...
class SimMemcmp(SimProcedure):
    ...
    def run(self, **kwargs):
        ...
        if buffer_entirely_inside_enclave(self.state, s1, size) \
                and buffer_entirely_inside_enclave(self.state, s2, size):
            # Only speed up memcpy if both src and dest are entirely inside the enclave.
            # If either is not fully inside the enclave, we must account for aepic-style issues and not speed up performance
...
def _enclave_handle_memset(state, dst, val, size):
    ...
    if val == 0:
        if SDKManager().addr_in_unmeasured_uninitialized_page(dst, size):
            ...
            logger.debug(
                f'SimMemset: Performing smart memset to {dst:#x} with {val:#x} with size of {size} by marking this as initialized.')
        else:
            logger.debug(
                f'SimMemset: Ignoring redundant memset to {dst:#x} with {val:#x} with size of {size} as it is in measured enclave memory (which is already zero-initialized).')
    else:
...
class SimRep(SimProcedure):
    ...
    def run(self, inst=None, **kwargs):
        ...
        if rep_count_concrete is not None:
            ...
            logger.warning(f'Reached a rep count @ addr {self.state.regs.rip} that appears to be symbolic. '
                            f'This can be a real issue! I will simulate the largest rep count possible and '
                            f'MAY COMPLETELY BREAK this state, but at least we get some plugin fireworks out of it '
                            f'if there is a real issue.')
            ...
            if SIM_REPS_REPORTER:
                SIM_REPS_REPORTER.report('Symbolic rep count',
                              self.state,
                              logger,
        ...
        if inst_stem == 'stos': # Form of stosb/w/d/q
            ...
            if buffer_entirely_inside_enclave(self.state, dst, size) and \
                buffer_entirely_inside_enclave(self.state, src, size):
