...
def run_ci(ctx: CIContext, config):
    ...
    async def control_eventloop():
        ...
        for binary_name in ci_binaries:
            command_name = f"{sys.executable} {base_path.resolve().as_posix()}/pandora.py explore -c {config} -n 0 {examples_folder}/{binary_name}"
            logger.info(f"Executing {command_name}")
            task_list.append(asyncio.create_subprocess_shell(command_name, stdout=subprocess.DEVNULL))
...
def check(ctx: CIContext, base_config_path="", ci_config_path=""):
    ...
    num_issues = {logging.DEBUG: 0, logging.INFO: 0, logging.WARNING: 0, logging.CRITICAL: 0}
    ...
    for file in ci_files:
        ...
        if len(baseline_issues) != len(ci_issues):
            logger.critical(f"Baseline and CI for {file} are different. Baseline log has {len(baseline_issues)} entries and newly generated CI has {len(ci_issues)} entries")
...
def fix(ctx: CIContext):
    ...
    if not fixed:
        ...
        if not fixable:
            ...
            stop_metadata = {"type": file_metadata["type"], "stop_timestamp": file_metadata["start_timestamp"]}
...
def main_callback(
    ...
    no_rerun_ci: bool = typer.Option(False, "--no-rerun", help="Whether to rerun the CI. Set to false to use the cached version."),
):
    """
